...
If the testcase is **responsive / multi-viewport** — (`testcase_category` is **`functional`** **and** the text matches Case Builder rule **3.5** / multi-viewport intent, **or** the name/description mentions desktop/tablet/mobile/viewport/หน้าจอ/responsive), **or** (`testcase_category` is **`visual`** **and** the goal is **viewport-only / no form fill** per Case Builder rule **2.6** item **(4)** — you MUST implement **viewport cycling** with a **screenshot + DOM capture at each size** (see `responsive_testing_rule`). **Viewport order is fixed:** **tablet → mobile → desktop** (set size, short wait, capture — then the next size). For typical “see the page at three sizes” goals, keep the script **minimal**: **`await page.waitForTimeout(1000)` once per viewport** (about **1 second** — do not stack long waits), capture artifacts — **do not** add heavy per-breakpoint interaction, overlap audits, or full usability matrices unless the testcase text explicitly demands deep responsive QA. For that **`visual`** viewport-only class (**2.6 (4)**), **do not** fill forms or click through wizards — **only** resize and capture.
</goal>
...
1. `await page.goto(testUrl)` then `await injectSmoothCursor(page)` as usual (one navigation for the whole test). **Viewport-only `visual` (Case Builder 2.6 (4)):** still `goto` + cursor setup; **skip** all form fills and step navigation.
