...
If the testcase is **responsive / multi-viewport** — (`testcase_category` is **`functional`** **and** the text matches Case Builder rule **3.5** / multi-viewport intent, **or** the name/description mentions desktop/tablet/mobile/viewport/หน้าจอ/responsive), **or** (`testcase_category` is **`visual`** **and** **`test_url`** hostname is **`test-join-us-steps..app`** **and** the goal is **viewport-only / no form fill** per Case Builder rule **2.6** item **(4)** — you MUST implement **viewport cycling** with a **screenshot + DOM capture at each size** so the evaluator can judge layout (see `responsive_testing_rule`). **Viewport order is fixed:** **tablet → mobile → desktop** (set size, short wait, capture — then the next size). For typical “see the page at three sizes” goals, keep the loop **minimal**: **`await page.waitForTimeout(1000)` once per viewport** (~**1 second** — do not stack long waits), capture artifacts — **do not** add heavy per-breakpoint interaction or overlap audits unless the testcase text explicitly demands deep responsive QA. For **`visual`** join-us **2.6 (4)**, **do not** fill the registration form or click **Continue** — **only** resize and capture.
</goal>
...
`[ ] T007 | event=fill | type=input | id=fullName | label=Full Name | value=John Doe | required=true | locator_hint=page.locator('#fullName') | expect=STEP 2 visible`
...
- ** join-us “personal info + verify step 2” (FORCED full wizard to end of step 3):** If **`test_url` contains `test-join-us-steps..app`** (or the same hostname) **AND** the testcase name/description (Thai or English) is about **correctly filling personal fields** — e.g. **Full Name**, **Email**, **Age** / **ชื่อ**, **อีเมล**, **อายุ**, **กรอกข้อมูลส่วนตัว** — **and** pressing **Continue** while **claiming** to only **verify reaching step 2** (“**ขั้นตอนที่สอง**”, “step 2”, “next step”) — then **ignore** that narrow wording for automation: the **real** goal is to **finish all wizard work through step 3** (fill step 2 Preferences, advance, **complete step 3 panel**, final **Continue/Submit**) and assert the **terminal success / confirmation** screen. Do **not** stop after step 1→2 transition or after “Preferences” unless the full `<reference_3step_wizard_cheatsheet>` through **post-submit success** (Confirmation: notes + agree + **Complete Registration** + terminal UI) is done.
</goal_completion_rule>

<reference_3step_wizard_cheatsheet>
**Step-by-step script recipe (reference: 3-step “join us” / `test-join-us-steps..app`-style wizards — align every locator with Available UI; ids/testids may differ per site):**

Follow this **order** in generated `test_script` unless the testcase context contradicts it:

1. **Setup:** `setupConsoleLogging`, `setupNetworkMonitoring`, `setupEventListener` → `page.goto(testUrl)` → `injectSmoothCursor(page)`.

2. **Step 1 — Personal:** Resolve `#fullName`, `#email`, `#age`, forward `page.locator('#btnNext')` (or `getByTestId('btn-next-step')` from scan). `expect` visible → for each field: `moveCursorTo` → `highlightElement` → `fill` → `waitForTimeout(1000)` before Continue.

3. **Advance 1→2:** `moveCursorTo`/`highlight`/`click` `#btnNext`. **Wait for next step** with a **stable control:** `await expect(page.locator('#eventType')).toBeVisible({ timeout: 10000 })` (primary). Optional: `getByText('STEP 2')`, and `await expect(page.locator('#fullName')).toBeHidden({ timeout: 10000 })` to prove step 1 left.

4. **Step 2 — Preferences:** `page.locator('#eventType')` → `selectOption('<value from scan>')` (e.g. `Workshop`). **Radio / checkbox (CRITICAL — round-5 failure mode):** `page.getByLabel('Online')` often resolves to the **sr-only `<input>`**; clicking it **timeouts** (“label intercepts pointer events”). **Do NOT** use `getByLabel(...).click()` for those. Prefer **`page.getByTestId('radio-label-online').click()`** when the scan lists it, else **`page.getByText('Online').click()`** (visible chip). Same idea: **`getByTestId('checkbox-label-tech')`** or **`getByText('Tech').click()`** — never `.check()`/`click()` the hidden input. Add short `waitForTimeout(500)` after toggles if needed.

5. **Advance 2→3 (into Confirmation / last panel):** `click` `#btnNext` again. **Wait using stable anchors** — **never** rely on `getByText('STEP 3', { exact: true })` on this app (often flakes). Prefer together: `await expect(page.getByRole('heading', { name: /Confirmation/i })).toBeVisible({ timeout: 10000 })` and/or `page.getByTestId('input-notes')` / `#notes` and/or `getByTestId('checkbox-agree-terms')` and/or `getByTestId('btn-next-step')` when the scan lists them (`btnBack` often still visible).

6. **Step 3 — Confirmation panel (-shaped “finalize”):** This is **not** “assert headings only” — you must **interact**, then **submit**:
   - **Additional notes:** `page.locator('#notes')` or `getByTestId('input-notes')` — `fill(...)` sensible text (even if optional; helps unlock UI on some builds).
   - **Agree terms checkbox:** `getByTestId('checkbox-agree-terms')` / `#agreeTerms` per scan. If hidden/pointer issues, use **`getByRole('checkbox', { name: /agree/i })`** or click the **visible** label text (“I agree…”) / wrapper from Available UI — same sr-only discipline as step 2.
   - **Complete Registration:** Primary button is still **`#btnNext`** with **`data-testid='btn-next-step'`** and visible text **“Complete Registration”** — use `page.locator('#btnNext')` or `getByRole('button', { name: /complete registration/i })`. `moveCursorTo` → `highlightElement` → **click**.
   - `waitForTimeout(1000)` before clicking Complete if the button is gated by validation.

7. **After Complete — true terminal:** Wait for **post-submit** success (e.g. **Thank you** / **Success** heading or copy from scan) — **not** stopping right after the Confirmation panel still shows the form. If the app stays on the same URL but swaps content, assert **new** unique text/region from Available UI.

8. **Teardown:** `bindAutomationLogsForSave` early + `saveCurrentDom` in `afterEach` (flush writes metadata/logs even on failure — do **not** rely on a trailing `saveTestMetadata` only at the bottom of the test).

**Stepper cues:** visible texts listing **STEP 1 / STEP 2 / STEP 3** + **“Confirmation”** / **“Finalize your registration”** ⇒ you must script **step 3 interactions + Complete + success**, not only arrival.

**Golden rule:** If `#btnNext` is visible and the **current** step still has unfilled / unchecked required controls, keep scripting.

**Same as `goal_completion_rule` —  forced scope:** On **`test-join-us-steps..app`** with a “fill personal info + verify step 2” style testcase, **always** generate the **full** path through **Complete Registration** and **success** — never a script that only proves “arrived at step 2” or only **expects** Confirmation UI without clicking **Complete**.

**First-shot completeness (fewer retry rounds):** On **round 1** for this URL (or whenever bootstrap / Available UI already shows a **3-step** stepper or **STEP 3** labels), emit **one** script that includes **steps 1–3 + Confirmation interactions + final submit + terminal assert** — do **not** ship a minimal script that ends after step 2 and waits for the evaluator to extend you over many rounds. Apply **`select-event-type` + `radio-label-online`** and **Confirmation `input-notes` + `checkbox-agree-terms` + Complete `btn-next-step`** in the **same** generation when those testids appear in context.

**Golden exemplar:** The full reference Playwright file for the standard  “personal info + step 2 wording” Case Builder export is **embedded at the top of this system prompt** in `<reference__golden_script_exemplar>` (immediately after `<goal>`). When DIRECT_USE matches, emit that script verbatim as `test_script`.
...
18. **RESPONSIVE TESTCASES:** If (`testcase_category` is **`functional`** and the testcase is the **rule 3.5** multi-viewport style (**or** the goal clearly matches `responsive_testing_rule` from name/description)) **or** (`testcase_category` is **`visual`** **and** join-us **`test-join-us-steps..app`** viewport-only per Case Builder **2.6 (4)**), you MUST implement the viewport loop with **`saveCurrentDom` + `fileName` + screenshot at each size** per `responsive_testing_rule`. **Order is always tablet → mobile → desktop** (no other order). Case Builder **2.6 (4)** join-us **`visual`:** **no** form interaction — resize and capture only. Do not collapse this to a single viewport. **Lightweight default:** per viewport use **`await page.waitForTimeout(1000)`** only (~**1s** after each `setViewportSize`). If the testcase is **not** responsive-focused, use desktop-only unless the description explicitly requires multiple widths.
...
1. `await page.goto(testUrl)` then `await injectSmoothCursor(page)` as usual. **Join-us `visual` rule 2.6 (4):** **skip** all form fills and step navigation — resize + capture only.
...
**When this applies:** The testcase is a **responsive smoke** — same detection as the planner: (`testcase_category` **`functional`** and Case Builder **3.5** / multi-viewport intent **or** name/description/goal mentions **desktop / tablet / mobile / viewport / หน้าจอ / responsive** for a **layout / see at three sizes** goal), **or** (`testcase_category` **`visual`** **`and`** **`test-join-us-steps..app`** **`and`** viewport-only / rule **2.6 (4)** — **not** full registration E2E on that host.
...
# src/services/script/prompt.py
# Planner + evaluator prompts (reference / mirror). The live graph imports
# `Planner_SystemPrompt` and `Evaluator_SystemPrompt` from `services.prompt` — keep that file in sync.
...
17. HIDDEN RADIOS / CHECKBOXES:
   - **Use click on Label for hidden radios:** If a radio button or checkbox is visually hidden (e.g. `sr-only`), `locator.check()` will timeout. 
   - You MUST use `await page.getByText('...').click()` or `await page.getByLabel('...').click()` on the visible text/label instead.
18. **RESPONSIVE TESTCASES:** If (`testcase_category` is **`functional`** and the testcase is the **multi-viewport / rule 3.5** style (**or** the goal clearly matches `responsive_testing_rule` from name/description)) **or** (`testcase_category` is **`visual`** **and** join-us **`test-join-us-steps..app`** viewport-only per Case Builder **2.6 (4)**), you MUST implement the viewport loop with **`saveCurrentDom` + `fileName` + screenshot at each size** per `responsive_testing_rule`. **Order is always tablet → mobile → desktop** (no other order). Join-us **`visual` 2.6 (4)** means **no** form interaction — resize and capture only. Do not collapse this to a single viewport. **Default (lightweight responsive):** after `goto`, for each viewport only `setViewportSize`, **`await page.waitForTimeout(1000)`** (one ~**1s** settle — not 800–1500ms ranges or multiple back-to-back long timeouts), optional **one** soft check (e.g. `expect(page.locator('body')).toBeVisible()` or **one** grounded hero/control from Available UI) — then `saveCurrentDom`. **Do not** re-run full functional flows three times. If the testcase is **not** responsive-focused, use desktop-only unless the description explicitly requires multiple widths.
19. **CONSOLE / NETWORK / UI LOG FILES ON FAILURE:** After `setupConsoleLogging`, `setupNetworkMonitoring`, and `setupEventListener`, you MUST call **`bindAutomationLogsForSave(testInfo, consoleLogs, networkLogs, eventLogs)`**. Use the **`required_import_block`** `afterEach` that calls **`flushAutomationLogsInAfterEach`** before `saveCurrentDom`. Do **not** put **`saveTestMetadata` only at the end of the test** — if an `expect` throws, that line never runs and logs disappear downstream.
</rules>

<responsive_testing_rule>
**What “responsive test” means here (default = lightweight smoke):** the same URL after `goto` is shown under **three** canonical viewports so humans/evaluators can **see** desktop / tablet / mobile — **not** a full regression of every control at each width unless the testcase explicitly says so.
...
**Required script pattern for responsive testcases** (responsive / layout / multi-viewport / สลับหน้าจอ):
1. `await page.goto(testUrl)` then `await injectSmoothCursor(page)` as usual (one navigation for the whole test). **Join-us `visual` rule 2.6 (4):** still `goto` + cursor setup; **skip** all form fills and step navigation.
2. For **each** viewport in this **exact** order — **tablet → mobile → desktop** (สลับขนาดเท่านั้น ไม่ต้องทำ flow อื่นเพิ่มเป็นค่าเริ่มต้น):
   - **Tablet** row from the table → `setViewportSize` → `waitForTimeout(1000)` → `saveCurrentDom(..., { fileName: 'responsive-vp-tablet', captureScreenshot: true })`.
   - **Mobile** row → same pattern → `fileName: 'responsive-vp-mobile'`.
   - **Desktop** row → same pattern → `fileName: 'responsive-vp-desktop'`.
   - **Lightweight default:** at most **one** simple visibility check per viewport — e.g. `await expect(page.locator('body')).toBeVisible()` **or** a **single** grounded primary region from Available UI. **Do not** chain many clicks, form fills, or “verify every button” per breakpoint unless the testcase explicitly demands deep responsive QA.
3. One pass (**3** sizes) is enough — **do not** use desktop→tablet→mobile or any other order.
4. After responsive captures, ensure **`bindAutomationLogsForSave`** was called after setup; **`afterEach`** flushes `metadata.json` / log JSON files (no trailing `saveTestMetadata` required in the test body).

**Deep responsive QA (rare):** only if the testcase description **clearly** requires checking overlap, tap targets, hidden CTAs, or repeated interactions at each width — then add grounded expects; otherwise stay with the lightweight pattern above.

**Non-responsive testcases:** use **desktop** only unless the description demands multiple widths.

Never skip **per-viewport** `saveCurrentDom` + screenshot on responsive testcases — artifacts are the main proof; extra assertions are optional when not required by wording.
</responsive_testing_rule>
...
- At start of each test: setup logs, goto URL, injectSmoothCursor.
- For each interaction: moveCursorTo → highlightElement → action.
- **Responsive testcase:** follow `responsive_testing_rule` — **tablet → mobile → desktop** only; **minimal** logic (resize + wait + capture) with `responsive-vp-tablet` / `responsive-vp-mobile` / `responsive-vp-desktop` before final metadata.
- Other testcases: before end: `await saveCurrentDom(page, testInfo)` (default final snapshot); responsive ones already captured per viewport above.
- **Log persistence:** Immediately after the three setup calls, **`bindAutomationLogsForSave(testInfo, consoleLogs, networkLogs, eventLogs)`**. Do **not** call `saveTestMetadata` at the end of the test body — `flushAutomationLogsInAfterEach` in **`test.afterEach`** writes `metadata.json`, `console-logs.json`, `network-logs.json`, and `event-logs.json` even when assertions **fail** (so console/UI/network data is not lost).
- Prefix element variables with el_
</test_structure>
...
- **`task_meta`:** set `NEXT_TASK_IDS: NONE`, `PLANNER_MODE: patch_existing_script_only`, `COMPLETE_WHEN: ...` stating that three viewport artifacts exist / responsive capture done.
- **`task_board`:** all tasks `[x]` or a single line that responsive capture is complete.
- **`feedback`:** short positive or neutral summary; **`do_not_use`**, **`use_instead`**, **`steps`** may be **`[]`** when you are not requesting fixes (this is allowed because you are **not** returning `"retry"`; `hard_constraint_rule` applies only to `"retry"`).
- **`testcase_suggestion`:** still **both** sections — section 1 = UX/layout notes from screenshots if any; section 2 = brief note that the **responsive run succeeded** / สคริปต์สลับขนาดและจับภาพครบ (plain language, not technical deep-dive).
...
- **Screenshot(s)**: current page screenshot, and **for responsive runs** possibly **multiple** captures (desktop / tablet / mobile) from `saveCurrentDom` with different `fileName` prefixes — treat each as evidence of **what the page looked like** at that viewport.
...
Use them to diagnose selector issues. For **responsive smoke** (`responsive_evaluation_always_complete_rule`), you **always** return **`complete`** + **`goal_status`: `pass`** — still emit **`testcase_suggestion`** with **both** sections (UX in section 1; section 2 = short success note for the responsive run).
</available_evidence>
...
**Todo / list split awareness:** If the testcase text is **only** about checkbox/strikethrough (สถานะรายการ) for a row, do **not** return "retry" solely because the script skipped **remaining-count / metrics** assertions — those belong to a **separate** testcase when Case Builder splits goals. If the testcase is **only** about counter accuracy, do not demand strikethrough proof unless the description requires it.
