...
var chunk_EXKYY76S_exports = {};
__export(chunk_EXKYY76S_exports, {
...
module.exports = __toCommonJS(chunk_EXKYY76S_exports);
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_5BZKOCL2.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_BJTR6QPO.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_BJTR6QPO.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_5BZKOCL2.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_PJ4PJ35Z.getConfig)({ datamodel: schemaResult.schemas, ignoreEnvVarErrors });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_I2DCNXW7.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_KNNOI5VB.mergeSchemas)({ schemas: schemaContext.schemaFiles });
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_I2DCNXW7.getDMMF)({
            datamodel: schemaContext.schemaFiles,
            previewFeatures
          });
...
function checkUnsupportedDataProxy({
  ...
  const url = (0, import_chunk_PJ4PJ35Z.resolveUrl)((0, import_chunk_PJ4PJ35Z.getEffectiveUrl)(schemaContext2.primaryDatasource));
