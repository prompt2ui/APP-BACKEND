...
var chunk_LKVPOSIP_exports = {};
__export(chunk_LKVPOSIP_exports, {
...
module.exports = __toCommonJS(chunk_LKVPOSIP_exports);
...
var import_chunk_LHBCJR4F = require("./chunk-LHBCJR4F.js");
var import_chunk_O3XIIOJR = require("./chunk-O3XIIOJR.js");
var import_chunk_EFX66NHL = require("./chunk-EFX66NHL.js");
var import_chunk_U3SMJT7S = require("./chunk-U3SMJT7S.js");
var import_chunk_ZCBEMBHR = require("./chunk-ZCBEMBHR.js");
var import_chunk_2Y55EHHW = require("./chunk-2Y55EHHW.js");
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_2Y55EHHW.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_EFX66NHL.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_EFX66NHL.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_2Y55EHHW.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_LHBCJR4F.getConfig)({ datamodel: schemaResult.schemas, ignoreEnvVarErrors });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_O3XIIOJR.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_U3SMJT7S.mergeSchemas)({ schemas: schemaContext.schemaFiles });
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_O3XIIOJR.getDMMF)({
            datamodel: schemaContext.schemaFiles,
            previewFeatures
          });
...
function checkUnsupportedDataProxy({
  ...
  const url = (0, import_chunk_LHBCJR4F.resolveUrl)((0, import_chunk_LHBCJR4F.getEffectiveUrl)(schemaContext2.primaryDatasource));
